...
class TextPreprocessing():
    def __init__(self, config):
        ...
        self.log = root_log.StandardLogger("TextPreprocessing")
    ...
    def positional_embedding(
            self, split_positional_encodings, vectorized_text, scalar=3):
        """Computes the positional embedding.
        ...
        for i in range(len(position_computed)):
            final_position_embedding.append(
                int(vectorized_text[i]) + position_computed[i])
    ...
    def print_results(
            self, attention_weights, threshold, vectorized_text,
    ...
    def __call__(self, input_text):
        ...
        attention_weights = self.SelfAttention(
            out_fin, out_fin, out_fin, len(out_fin))
        ...
        for i in range(len(attention_weights)):
            ...
            final_output_array.append(
                [attention_weights[i],
                 self.vectorize_layer.get_vocabulary()[vec[i]],
                 stat])
