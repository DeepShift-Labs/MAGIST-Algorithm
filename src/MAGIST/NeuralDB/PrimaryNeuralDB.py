...
class NeuralDB():
    ...
    def recreate_db(self):
        ...
        self.log.warning(
            "NeuralDB is about to reset and recreate all Databases and tables. Proceeding in 5 seconds...")
        ...
        for d in self.db_string:
            if d == "vision":
                ...
                self.log.info(
                    "Vision database is included in NeuralDB search.")
            ...
            if d == "common":
                ...
                self.log.info(
                    "Common database is included in NeuralDB search.")
        ...
        try:
            if self.vision is None:
                self.log.warning(
                    "Vision database was not included from NeuralDB search.")
        ...
        try:
            if self.nlp is None:
                self.log.warning(
                    "NLP database was not included from NeuralDB search.")
        except AttributeError:
            self.log.warning(
                "NLP database was not included from NeuralDB search.")
        ...
        try:
            if self.common is None:
                self.log.warning(
                    "Common database was not included from NeuralDB search.")
        except AttributeError:
            self.log.warning(
                "Common database was not included from NeuralDB search.")
    ...
    def insert_obj_location(self, obj_name, obj_location):
        ...
        """
        self.log.info(
            f"Inserting object location: {obj_name} - {obj_location}")
        self.obj_location.insert_one(
            {"obj_name": obj_name, "obj_location": obj_location})
    ...
    def insert_obj_obj_relation(self, obj_name, second_obj_name):
        ...
        self.log.info(
            f"Inserting object object relation: {obj_name} - {second_obj_name}")
        self.obj_obj_relation.insert_one(
            {"obj_name": obj_name, "second_obj_name": second_obj_name})
    ...
    def insert_obj_users(self, obj_name, user_name):
        ...
        self.obj_users.insert_one(
            {"obj_name": obj_name, "user_name": user_name})
    ...
    def insert_word_desc(self, word_name, word_desc):
        ...
        """
        if word_desc is not None:
            self.log.info(
                f"Inserting word description: {word_name} - {word_desc}")
            self.word_desc.insert_one(
                {"word_name": word_name, "word_desc": word_desc})
    ...
    def insert_word_location(self, word_name, word_location):
        ...
        self.log.info(
            f"Inserting word location: {word_name} - {word_location}")
        self.word_location.insert_one(
            {"word_name": word_name, "word_location": word_location})
    ...
    def insert_word_obj_relation(self, word_name, obj_name):
        ...
        self.log.info(
            f"Inserting word object relation: {word_name} - {obj_name}")
        self.word_obj_relation.insert_one(
            {"word_name": word_name, "word_relation": obj_name})
    ...
    def search_obj_details(self, obj_name):
        ...
        for d in self.dbs:
            ...
            for i in d.list_collection_names():
                ...
                for j in self.vision[i].find(
                        {"obj_name": re.compile(rf"\b{obj_name}\b", re.IGNORECASE)}):
                    self.log.info(f"        ===> {j}")
                    data.append(j)
        return data
    ...
    def search_obj_desc(self, keyword):
        ...
        for d in self.dbs:
            ...
            for i in d.list_collection_names():
                self.log.info(f"    ===> Collection: {i}")
                for j in self.vision[i].find(
                        {"obj_desc": re.compile(rf"\b{keyword}\b", re.IGNORECASE)}):
                    self.log.info(f"        ===> {j}")
                    data.append(j)
        return data
    ...
    def search_obj_location(self, location):
        ...
        for d in self.dbs:
            ...
            for i in d.list_collection_names():
                self.log.info(f"    ===> Collection: {i}")
                for j in self.vision[i].find(
                    {"obj_location": re.compile(
                        rf"\b{location}\b", re.IGNORECASE)}):
                    self.log.info(f"        ===> {j}")
                    data.append(j)
        return data
    ...
    def search_obj_user(self, user):
        ...
        for d in self.dbs:
            ...
            for i in d.list_collection_names():
                self.log.info(f"    ===> Collection: {i}")
                for j in self.vision[i].find(
                        {"user_name": re.compile(rf"\b{user}\b", re.IGNORECASE)}):
                    self.log.info(f"        ===> {j}")
                    data.append(j)
        return data
    ...
    def search_word_details(self, word):
        ...
        for d in self.dbs:
            ...
            for i in d.list_collection_names():
                self.log.info(f"    ===> Collection: {i}")
                for j in self.vision[i].find(
                        {"word_name": re.compile(rf"\b{word}\b", re.IGNORECASE)}):
                    self.log.info(f"        ===> {j}")
                    data.append(j)
        return data
    ...
    def search_word_desc(self, keyword):
        ...
        for d in self.dbs:
            ...
            for i in d.list_collection_names():
                self.log.info(f"    ===> Collection: {i}")
                for j in self.vision[i].find(
                        {"word_desc": re.compile(rf"\b{keyword}\b", re.IGNORECASE)}):
                    self.log.info(f"        ===> {j}")
                    data.append(j)
        return data
    ...
    def search_word_relation(self, relation):
        ...
        for d in self.dbs:
            ...
            for i in d.list_collection_names():
                self.log.info(f"    ===> Collection: {i}")
                for j in self.vision[i].find(
                    {"word_relation": re.compile(
                        rf"\b{relation}\b", re.IGNORECASE)}):
                    self.log.info(f"        ===> {j}")
                    data.append(j)
        return data
    ...
    def search_entire_db(self, term):
        ...
        for d in self.dbs:
            ...
            for c in d.list_collection_names():
                ...
                exec(
                    f"db_col_search = self.client.{d.name}.{c}",
                    self._locals_search)
                ...
                for key in keys:
                    ...
                    search = db_col_search.find(
                        {key: re.compile(rf"\b{term}\b", re.IGNORECASE)})
        ...
        for d in self.dbs:
            for i in d.list_collection_names():
                ...
                replic = db_col.aggregate([  # Cursor with all duplicated documents
                    {'$group': {
                        # Duplicated field
                        '_id': {repeated_val: f'${repeated_val}'},
                        'uniqueIDs': {'$addToSet': '$_id'},
                        'total': {'$sum': 1}
                    }
                    },
                    {'$match': {
                        # Holds how many duplicates for each group, if you need
                        # it.
                        'total': {'$gt': 1}
                    }
                    }
                ...
                for i in replic:
                    for idx, j in enumerate(
                            i['uniqueIDs']):  # It holds the ids of all duplicates
