...
class ImageSlicer:
    ...
    def __init__(self, config):
        ...
        self.log = root_log.StandardLogger("ImageSlicer")
    ...
    def resizer(self, size, base_path):
        ...
        for f in tqdm(files):  # for i in range(image_count + 1):
            try:
                # str(base_path) + "/Frame(" + str(i) + ").jpg"
                image_path = os.path.join(base_path, str(f))
                try:
                    ...
                    self.log.warning(
                        "Unidentified image: {}".format(image_path))
                ...
                if error == ValueError:
                    self.log.warning(
                        "Image Type Not Supported: {}".format(image_path))
    ...
    def coordinate_compute(self, input_size, individual_output_size):
        ...
        self.log.info(
            "Computing coordinates for {}x{} image. Preliminary checks complete.".format(
                l,
                w))
        ...
        for f in tqdm(files):  # for i in range(image_count + 1):
            ...
            for c in range(len(coordinates)):
                ...
                save_location = os.path.join(
                    end_path, img_class, "Frame" + str(counter) + str(c) + ".jpg")
    ...
    def image_integrity_verification(self, path, delete_invalid=True):
        ...
        if delete_invalid:
            self.log.info(
                "Deleted {} invalid files of {}.".format(
                    bad_files, bad_files + good_files))
        else:
            self.log.info(
                f'{bad_files} invalid files of {bad_files + good_files} were found.')
